...
python train.py task=bash_AnymalTerrain_NeuroRL_exp_generate_pca \
  ...
  task.env.specifiedCommandVelocityN.linear_x=3 \
  task.env.specifiedCommandVelocityN.linear_y=3 \
  task.env.specifiedCommandVelocityN.yaw_rate=3 \
  task.env.specifiedCommandVelocityN.n_copies=13 \
  task.env.output.export_data=True \
  +output_path='../../neuro-rl/neuro_rl/data/anymal_uvr_27speed'